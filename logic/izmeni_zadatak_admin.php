...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $id = $_POST['dropdown_id'];
    $naslov = $_POST['naslov'];
    $opis = $_POST['opis'];
    $lista_izvrsioca = $_POST['lista_izvrsioca'];
    $rukovodilac = $_POST['dropdown_ruk'];
    ...
    if (empty($fajl)) {
        ...
        $fajlovi = explode (',', $naziv_fajla);

        foreach ($fajlovi as $item) {
    ...
    try {
        Zadatak::izmeni_zadatak($id, $naslov, $opis, $lista_izvrsioca, $rukovodilac, $rok_izvrsenja, $prioritet, $id_grupe, $fajl);
        header('Location: ../pages/tipovi_korisnika/radnje_administrator/izmeni_zadatak.php?status=uspesno');
        exit();
    } catch (Exception $e) {
        header('Location: ../pages/tipovi_korisnika/radnje_administrator/izmeni_zadatak.php?status=greska');
        exit();
    }
}
