...
if (isset($_GET['status'])) {
    if ($_GET['status'] == 'uspesno') {
        $poruka_uspesno = '<br>Zadatak je uspešno izmenjen.<br><br>';
    } else if ($_GET['status'] == 'greska') {
        $poruka_greska = '<br>Došlo je do greške pri izmeni zadatka.<br>Pokušajte ponovo.<br><br>';
    }
}
...
<form id="napravi" action="../../../logic/izmeni_zadatak_admin.php" method="POST" enctype="multipart/form-data">
    <p id="status_uspesno"><?= $poruka_uspesno ?></p>
    <p id="status_greska"><?= $poruka_greska ?></p>

    <label for="id">Odaberite zadatak koji želite da izmenite:</label>
    <select name="dropdown_id" id="id" required>
    ...
    <select name="dropdown_ruk" id="rukovodilac" required>
    ...
    <input type="submit" value="Izmeni zadatak" class="button">
</form>
<footer>
